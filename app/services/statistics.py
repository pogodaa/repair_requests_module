...
def load_requests_from_excel() -> List[Request]:
...
def calculate_avg_repair_time(requests: List[Request]) -> float:
...
def count_completed_requests(requests: List[Request]) -> int:
    return sum(1 for r in requests if r.requestStatus == "Готова к выдаче")

def get_top_equipment_types(requests: List[Request], top_n: int = 3) -> dict:
